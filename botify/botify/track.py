...
class Catalog:
    ...
    def load(self, catalog_path, top_tracks_path, tracks_with_diverse_recs_path):
        ...
        self.app.logger.info(f"Loading tracks with diverse recs from {tracks_with_diverse_recs_path}")
    ...
    def upload_tracks(self, redis_tracks, redis_tracks_with_diverse_recs):
        ...
        self.app.logger.info(f"Uploading tracks with diverse recs to redis")
        for track in self.tracks_with_diverse_recs:
            redis_tracks_with_diverse_recs.set(track.track, self.to_bytes(track))
        self.app.logger.info(f"Uploaded {len(self.tracks_with_diverse_recs)} tracks")
    ...
    def upload_recommendations(self, redis, recommendations_file_config="RECOMMENDATIONS_FILE_PATH"):
        recommendations_file_path = self.app.config[recommendations_file_config]
